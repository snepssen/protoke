...
#: How stem separators name the vocal they extract. UVR5 writes
#: "<track>_(Vocals).wav"; others use a plain suffix.
VOCAL_MARKERS = ("vocals", "vocal", "vox")


def find_vocal_stem(audio_path):
    """A sibling vocal-only render of this track, if one was made.

    Nothing here separates stems. But a mouth driven by a lyric sheet cannot
    see wordless singing, and picking the voice out of a full mix is guesswork
    — so if the separation has already been done, use it.
    """
    # Underscores survive canonical_stem (they are word characters), and
    # separators put them everywhere, so flatten them for this comparison.
    def flat(path):
        return canonical_stem(path).replace("_", "")

    wanted = flat(audio_path)
    ...
    for name in names:
        path = os.path.join(folder, name)
        if not (os.path.isfile(path) and name.lower().endswith(AUDIO_EXTS)):
            continue
        stem = flat(path)
        if stem == wanted:
            continue
        for marker in VOCAL_MARKERS:
            if not stem.endswith(marker):
                continue
            body = stem[:-len(marker)]
            if body == wanted:
                return path
            # Separators often prefix a track number. Only a number may sit
            # in front — matching on containment alone pairs "song.wav" with
            # "other song_(Vocals).wav".
            if body.endswith(wanted) and body[:-len(wanted)].isdigit():
                return path
    return None
...
def resolve_track_assets(audio_path):
    ...
    return {
        ...
        "vocals": find_vocal_stem(audio_path),
    }
