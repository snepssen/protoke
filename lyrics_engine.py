...
def load_transcript(json_path):
    """Return ``(words, preset_lines)`` for any transcript this tool writes.

    A corrected transcript carries its own line structure — the reference
    lyrics decide where lines break — and that shape is not a plain word list,
    so anything reading a transcript has to come through here rather than
    calling :func:`load_words` directly.
    """
    with open(json_path, "r", encoding="utf-8") as handle:
        data = json.load(handle)
    if isinstance(data, dict) and "lines" in data:
        preset_lines = [line["words"] for line in data["lines"]
                        if line.get("words")]
        return [word for line in preset_lines for word in line], preset_lines
    return load_words(json_path), None
...
def transcript_to_ass(json_path, ass_path, colors, font="Arial Black",
    ...
    words, preset_lines = load_transcript(json_path)
    if not words:
